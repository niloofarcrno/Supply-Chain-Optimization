...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct  1 01:04:50 2023
...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct  1 01:04:34 2023
...
model.Budget= pyo.Var(t_set, bounds=(0,None))
Budget= model.Budget
...
+(1-alpha)*sum(VCC[i]*C[i,t] for i in i_set for t in t_set)
+(1-alpha)*sum(VCB * P[l,t]  for l in l_set for t in t_set)
+sum(FCC*Y[j] for j in j_set)
+(1-alpha)*sum(FBC*X[l] for l in l_set)
+(1-alpha)*sum(FCL[i]*Z[i] for i in i_set)
...
# budget constraint
model.budget1 = pyo.Constraint(i_set, rule=lambda model,t: (
    alpha*(sum(VCB * P[l,t]  for l in l_set) + sum(FCL[i]*Z[i] for i in i_set) + sum(VCC[i]*C[i,t] for i in i_set) + sum(FBC*X[l] for l in l_set))<= Budget[t]
))
#Budget maximum

model.budget2 = pyo.Constraint(expr=sum(Budget[t] for t in t_set) <= BudgetT)
...
for t in t_set:
        print(f"Budget[{t}] = {model.Budget[t].value}")
...
print ("totalcost", totalinventory+investcosttotal+bioethanol_prod+harvesting_costs+total_transp_value-Budget)
...
with pd.ExcelWriter('Variable goal.xlsx') as writer:
